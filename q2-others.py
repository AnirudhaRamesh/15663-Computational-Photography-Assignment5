...
# getting mean
# this is largely the same for all other objects barring dir1 for the others..
# basedir = './data-dump/mug/'
# dir_list = ['dir'+str(i)+'/' for i in range(1,9)]
# ims = ['capt0003.tiff', 'capt0005.tiff', 'capt0007.tiff']

# for dli in dir_list : 
#     mean_img = np.zeros((4000,6000,3))
#     for imf in ims :
#         filename = basedir + dli + imf 
#         im = cv2.imread(filename,-1)[8:-8,8:-8]
#         mean_img += im / len(ims)
#     mean_filename = filename.replace('.tiff','_mean.tiff')
#     cv2.imwrite(mean_filename, mean_img.astype('uint16'))

# basedir = './data-dump/mug/'
# outputdir = 'mug-means/'
# dir_list = ['dir'+str(i)+'/' for i in range(1,9)]
# ims = 'capt0007_mean.tiff'
# for dli in dir_list :
#     filename = basedir + dli + ims 
#     im = cv2.imread(filename,-1)
#     outfile = basedir + outputdir + ims.replace('_mean.tiff','_'+str(dli[:-1])+'_mean.tiff') 
#     print(outfile)
#     cv2.imwrite(outfile, im.astype('uint16'))
...
# def loadData(path = "./data-dump/plant/plant-means/"):
def loadData(path = "./data-dump/robot/robot-means/"):
# def loadData(path = "./data-dump/bottle-opener/bottle-opener-means/"):
# def loadData(path = "./data-dump/robot/robot-means/"):
    ...
    for i in range(1,8) : 
        image_name = path + 'capt0007_dir' + str(i) + '_mean.tiff'
        img_rgb = cv2.imread(image_name, -1).astype('uint16')[::8,::8,:]
#         img_rgb = img_rgb[:,550//2:980//2,:] # for plant
        img_rgb = img_rgb[190:380,225:390,:] # for robot and bottle opener
#         img_rgb = img_rgb[190:380,250:470,::-1] # for mug
...
# Applying GBR inversion transform. Flip not needed here, but still added to make depth map easier to discern.
Gf = np.array([[1,0,0],[0,1,0],[0,0,-1]])
Be_enf_int = Gf @ Be_enf_int 
albedos_enf_int, normals_enf_int = estimateAlbedosNormals(Be_enf_int)
albedoIm_int_enf, normalIM_int_enf = displayAlbedosNormals(albedos_enf_int, normals_enf_int, s, 'PseudoNormals integratability enforced after inversion of direction')
...
plt.imshow(surface_poisson)
plt.show()
plot_surface_ls(surface_poisson, False)
plt.show()

surface_frankot = estimateShape(normals_enf_int, s, 1)
plt.imshow(surface_frankot)
plt.show()
plot_surface_ls(surface_frankot, False)
plt.show()
...
# lighting the object 
# verifying with known lighting
# From straight-right top
# if you get empty image, flip the normal sign
ndotl = np.dot(-normalIM_int_enf,Le[:,0])
rad_field = albedoIm_int_enf * ndotl
rad_field[rad_field < 0] = 0
plt.imshow(rad_field)
plt.show()

# From left side
ndotl = np.dot(-normalIM_int_enf,Le[:,2])
rad_field = albedoIm_int_enf * ndotl
rad_field[rad_field < 0] = 0
plt.imshow(rad_field)
plt.show()

# Custom light, far right
L_custom = np.array([10,1,1]) 
L_custom = L_custom / np.linalg.norm(L_custom)
ndotl = np.dot(normalIM_int_enf,L_custom)
rad_field = albedoIm_int_enf * ndotl
rad_field[rad_field < 0] = 0
plt.imshow(rad_field)
plt.show()
